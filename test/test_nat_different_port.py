...
def test_dynamic_nat(config_op, interface="GigabitEthernet0/0/2"):
    """测试动态NAT配置"""
    print(f"\n2. 测试动态NAT配置在接口 {interface}...")
...
def verify_nat_config(config_op, interface="GigabitEthernet0/0/2"):
    """验证NAT配置"""
    print(f"\n3. 验证NAT配置(接口 {interface})...")
    ...
    try:
        ...
        # 进入接口
        config_op.connection.send_command(f"interface {interface}", expect_string=r"\[.*\]")
...
def main():
    ...
    # 测试动态NAT配置在新接口上
    nat_result = test_dynamic_nat(config_op, interface)
    
    # 验证NAT配置
    verify_result = verify_nat_config(config_op, interface)
