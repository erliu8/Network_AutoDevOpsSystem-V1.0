...
class BatchConfigWindow(QWidget):
    ...
    def connect_signals(self):
        """连接控制器信号"""
        self.log("[DEBUG] 连接控制器信号")
        if hasattr(self.controller, 'connection_result'):
            self.controller.connection_result.connect(self.connection_status)
            self.log("[DEBUG] 已连接connection_result信号")
        
        if hasattr(self.controller, 'config_result'):
            self.controller.config_result.connect(self.config_status)
            self.log("[DEBUG] 已连接config_result信号")
        
        if hasattr(self.controller, 'log_message'):
            self.controller.log_message.connect(self.log)
            self.log("[DEBUG] 已连接log_message信号")
    ...
    def init_ui(self):
        ...
        # 添加测试信号按钮
        self.test_signal_btn = QPushButton("测试信号传递")
        self.test_signal_btn.setToolTip("测试信号传递机制，验证UI和线程间的通信")
        self.test_signal_btn.clicked.connect(self.test_signals)
        thread_layout.addWidget(self.test_signal_btn)
    ...
    def apply_config(self):
        ...
        try:
            ...
            if reply == QMessageBox.Yes:
                ...
                # 设置一个安全QTimer，确保UI线程能够在信号到达前不会被卡住
                QTimer.singleShot(15000, self.check_result_status)
    ...
    def check_config_timeout(self):
        ...
        if int(elapsed) % 15 == 0 and int(elapsed) > 0:
            self.log(f"[DEBUG] 配置操作进行中，已用时 {int(elapsed)} 秒")
            
            # 查看此时任务线程状态
            import threading
            self.log(f"[DEBUG] 当前活动线程数: {threading.active_count()}")
            for thread in threading.enumerate():
                if "BatchConfig" in thread.name:
                    self.log(f"[DEBUG] 发现配置线程: {thread.name}, 状态: {'活动' if thread.is_alive() else '已结束'}")
        
        # 超过30秒视为超时 - 降低超时时间方便测试
        if elapsed > 30:
            self.timeout_timer.stop()
            self.log("[DEBUG] 警告: 配置操作已超过30秒，可能已经卡住")
            self.log("正在尝试恢复...")
            ...
            for thread in threading.enumerate():
                if "BatchConfig" in thread.name:
                    ...
                    self.log(f"[DEBUG] 检测到活动的配置线程: {thread.name}, 状态: {'活动' if thread.is_alive() else '已结束'}")
            
            if active_threads:
                self.log("[DEBUG] 检测到配置线程仍在运行，但UI未收到结果信号")
                # 手动触发配置成功
                self.config_status(True, "配置似乎已完成，但未收到结果信号")
            else:
                self.log("[DEBUG] 未检测到活动的配置线程，操作可能已失败")
                # 手动触发配置失败
                self.config_status(False, "配置操作超时")

    def config_status(self, success, message):
        """更新配置状态"""
        self.log(f"[DEBUG] 收到配置状态回调: 成功={success}, 消息={message}, 时间: {time.strftime('%H:%M:%S')}")
        
        # 停止超时计时器，如果存在
        if hasattr(self, 'timeout_timer') and self.timeout_timer.isActive():
            self.log("[DEBUG] 停止超时计时器")
            self.timeout_timer.stop()
        
        # 移除进度标签，如果存在
        if hasattr(self, 'progress_label') and self.progress_label is not None:
            try:
                self.log("[DEBUG] 移除进度标签")
                self.layout().removeWidget(self.progress_label)
                self.progress_label.deleteLater()
                self.progress_label = None
            except Exception as e:
                self.log(f"[DEBUG] 移除进度标签时出错: {str(e)}")
        
        # 启用应用按钮
        self.log("[DEBUG] 重新启用应用按钮")
    ...
    def test_signals(self):
        ...
        if self.controller:
            # 添加进度标签
            self.progress_label = QLabel("正在测试信号，请稍候...")
            self.progress_label.setStyleSheet("color: blue; font-weight: bold;")
            self.layout().addWidget(self.progress_label)
            ...
            # 启动测试
            self.controller.test_simple_signals()
        else:
            self.log("错误: 控制器未初始化，无法测试信号")